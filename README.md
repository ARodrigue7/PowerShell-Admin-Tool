...
*   `AdminTool.ps1`: The main graphical user interface (WPF application). Handles UI layout, event loop, target resolution, background job starting, and result aggregation.
*   `functions.psm1`: The PowerShell script module containing all custom administrative functions (e.g., processes, services, connections, scheduled tasks, startup folders, etc.).
